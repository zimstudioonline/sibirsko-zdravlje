 ...
 * ali sa unapred popunjenom porukom za konkretan proizvod preko `text`
 * parametra. Viber to zvanično ne garantuje za `viber://chat` (nema
 * dokumentovan parametar za draft poruku) — u praksi radi na većini Android
 * uređaja sa instaliranim Viberom, na iOS-u i desktopu se samo otvori razgovor
 * bez teksta. Isti "best effort" obrazac kao no-cors upit forma.
 */
export function ProductViberButton({ productName }: { productName: string }) {
  ...
  const href = `viber://chat?number=%2B${number}&text=${encodeURIComponent(message)}`;
