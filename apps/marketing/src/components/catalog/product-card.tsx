...
export function ProductCard({ product }: { product: Product }) {
  return (
    <Card className="flex h-full flex-col overflow-hidden py-0">
      <Link href={`/katalog/proizvod/${product.slug}`} className="contents">
      ...
      <CardContent className="flex flex-1 flex-col gap-2 px-4 pt-1 pb-4">
        ...
        <Link href={`/katalog/proizvod/${product.slug}`}>
          <h3 className="font-medium text-ink text-sm leading-snug hover:text-primary">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-2">
          ...
          <RequestInquiryButton productName={product.name} />
        </div>
      </CardContent>
    </Card>
  );
